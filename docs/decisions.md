...
- **Zero-dependency drivers + inlined stricli app** — workspace driver packages remain zero external runtime dependencies; command orchestration in `@aibridge/cli` uses `@stricli/core@1.3.0` inlined via `tsdown` into `skills/ai-bridge/scripts/cli.mjs`.
