...
- **`node packages/cli/src/cli.ts subagent`** → delegate a self-contained task to another model through the canonical registry: default **`xai-grok/grok-4.5`** via the **Grok CLI** (own xAI login; ~30 req/min + ~1k msgs/day caps, one run at a time); **`google-antigravity/gemini-3.6-flash`** via the **Antigravity CLI** (`agy`, own Antigravity login) when grok is capped/dead; **`openai-codex/gpt-5.6-sol`** via the **Codex CLI** (own ChatGPT login); last-resort **`anthropic-claude/sonnet`** / **`anthropic-claude/opus`** via the **claude CLI** (bill the claude CLI's Claude subscription — last resort for agents whose own budget that is).
