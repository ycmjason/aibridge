...
import { type AgyQuotaSnapshot, fetchAgyQuota } from '../../lib/agyQuota.ts';
import { type ClaudeQuotaSnapshot, fetchClaudeQuota } from '../../lib/claudeQuota.ts';
import { type CodexQuotaSnapshot, fetchCodexQuota } from '../../lib/codexQuota.ts';
...
export default async function quotaImpl(this: LocalContext, flags: QuotaFlags): Promise<void> {
  // Fetch all backends concurrently; one failing must not hide the others.
  // (claude shells out to `claude -p "/usage"` — the slowest leg at a few
  // seconds — but it runs in parallel with the two HTTP fetches.)
