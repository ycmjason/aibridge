...
import { DEFAULT_MODEL, formatUnknownModelError, resolveModel } from '../../lib/models.ts';
import { runCaptured } from '../../lib/proc.ts';
...
export function parseReviewVerdict(response: string): ReviewVerdictResult {
  ...
  // The contract says the final answer IS the verdict line, but backends
  // narrate: grok has been observed prepending progress prose to the final
  // message — sometimes without even a newline before the verdict. So: try
  // the first line, then the last, then fall back to the LAST full
  // "FINDINGS: c critical, m major, n minor" triple anywhere in the blob.
...
export default async function review(this: LocalContext, flags: ReviewFlags): Promise<void> {
  ...
  // Mode detection before preflight / model spend. `git diff --quiet` exits 0
  // (clean) or 1 (dirty); anything else is a real git error (e.g. a typo'd
  // --base ref) and must not be silently read as "dirty".
  ...
  // The verdict line is only trustworthy alongside the full report — a missing
  // or empty report file means the reviewer broke the contract.
  ...
  if (verdictResult.kind === 'unparseable') {
    // Contract: still surface the answer + paths on stdout so the orchestrator
    // can recover; the non-zero exit is what signals the broken contract.
    this.process.stderr.write(
