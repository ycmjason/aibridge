...
// Antigravity's installed-app OAuth client (not a secret in the installed-app
// flow); overridable if Google ever rotates it.
...
export interface AgyModelQuota {
  ...
  /** 0..1; 0 means exhausted. Undefined when the API omits it. */
  readonly remainingFraction: number | undefined;
  readonly exhausted: boolean;
  /** ISO timestamp when the quota window resets, if known. */
...
export interface AgyQuotaSnapshot {
  ...
  /**
   * Per-GROUP windows (weekly + 5h) from v1internal:retrieveUserQuotaSummary —
   * the same data agy's /quota TUI shows. Quota is shared per group (all
   * Gemini tiers drain together), so this is the planning view.
   */
  readonly groups: readonly AgyQuotaGroup[];
  /** Per-model binding-window view from fetchAvailableModels. */
...
export interface AgyQuotaBucket {
  ...
  /** "5h" | "weekly" as reported by the API. */
...
export function parseQuotaGroups(
  ...
  return groups.map(g => ({
    ...
    buckets: (g.buckets ?? []).map(b => ({
      ...
      window: b.window ?? '?',
      // Proto3 zero-omission again: an exhausted bucket omits the fraction.
...
/** Internal/feature models that aren't turn-executing chat models. */
...
export function parseModels(modelsById: Record<string, RawModelInfo>): AgyModelQuota[] {
  ...
  for (const [modelId, m] of Object.entries(modelsById)) {
    ...
    // Proto3 JSON omits zero values: an exhausted model's quotaInfo carries
    // only resetTime — a missing remainingFraction (with quotaInfo present)
    // MEANS 0, not unknown. Same for isExhausted:false being absent.
...
/**
 * Fetch the current agy quota snapshot. Cached for 60s (matters only for a
 * long-lived embedder; a one-shot CLI run never hits the cache).
 * Throws on any failure — callers treat quota as UNKNOWN and fall back.
 */
...
/**
 * Find quota for a model by the display label agy uses (e.g.
 * "Gemini 3.5 Flash (Low)") or by cloudcode model id. Multiple ids can share
 * a label; any exhausted match counts as exhausted (they share the pool).
 */
