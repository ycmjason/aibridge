import type { AgyQuotaSnapshot } from './agyQuota.ts';
import { fetchAgyQuota, findModelQuota } from './agyQuota.ts';
import type { CodexQuotaSnapshot } from './codexQuota.ts';
import { fetchCodexQuota } from './codexQuota.ts';
...
export function evaluateAgyPreflight(
  ...
  if (quota.exhausted) {
    ...
    if (!resetAt) {
      // Fall back to soonest resetTime among Gemini group buckets
...
export async function preflightModel(resolved: ResolvedModel): Promise<PreflightVerdict> {
  ...
  if (resolved.spec.backend !== 'agy') {
    // Only agy and codex have quota preflight probes.
    return { ok: true };
  }
