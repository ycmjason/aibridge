...
export async function probe(run: typeof runCaptured = runCaptured): Promise<Availability> {
  try {
    ...
    if (res.timedOut) {
      return { ok: false, error: 'ai-bridge: "agy" timed out probing version.' };
    }
    const line = (res.stdout || res.stderr).split('\n')[0]?.trim();
    if (line && line.length > 0) {
      return { ok: true, version: line };
    }
    return { ok: false, error: `ai-bridge: "agy" returned no version output. ${INSTALL_HINT}` };
  } catch (err) {
    if (isNotFound(err)) {
      return { ok: false, error: `ai-bridge: "agy" not found on PATH. ${INSTALL_HINT}` };
    }
    return { ok: false, error: `ai-bridge: failed to probe agy: ${(err as Error).message}` };
  }
}
